...
import (
	...
	"github.com/sanrentai/crypto/sm3"
)
...
func MarshalCipher(in []byte, cipherTextType Sm2CipherTextType) ([]byte, error) {
	...
	c2Len := len(in) - (1 + byteLen*2) - gm3.DigestLength
	c2 := make([]byte, c2Len)
	c3 := make([]byte, gm3.DigestLength)
	pos := 1
	...
	if cipherTextType == C1C2C3 {
		...
		pos += c2Len
		copy(c3, in[pos:pos+gm3.DigestLength])
